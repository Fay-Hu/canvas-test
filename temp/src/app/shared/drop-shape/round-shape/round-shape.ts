...
export class RectMetaball {
	...
	equation(tx: number, ty: number): number {
		...
		let
			...
			oy = y + h / 2,
			alpha = Math.atan(h / w),
			theta = Math.atan(Math.abs(oy - ty) / Math.abs(tx - ox));

		if ((theta >= alpha && theta <= Math.PI - alpha) || (theta >= alpha + Math.PI && theta <= 2 * Math.PI - alpha)) {
			return h / Math.abs(2 * (ty - oy));
		} else {
			return w / Math.abs(2 * (tx - ox));
		}
	}
}
